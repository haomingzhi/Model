...
- (void)setup {
    ...
    _scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(CGRectGetMaxX(_leftShadowView.frame), 0, CGRectGetWidth(self.bounds)  - CGRectGetWidth(_managerMenusButton.frame), CGRectGetHeight(self.bounds))];
...
- (UIButton *)getButtonWithMenu:(XHMenu *)menu {
    ...
    UIButton *button = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, buttonSize.width +[self menuButtonPadding], buttonSize.height)];
...
- (void)reloadData {
    ...
    self.indicatorView.backgroundColor = self.markColor;
    // layout subViews
    ...
    for (XHMenu *menu in self.menus) {
        ...
        CGFloat buttonX = 0;
        if (index) {
            //            buttonX = [self menuButtonPadding] + CGRectGetMaxX(((UIButton *)(self.menuButtons[index - 1])).frame);
            buttonX =   CGRectGetMaxX(((UIButton *)(self.menuButtons[index - 1])).frame);
        } else {
...
//获取间隔，>=kXHMenuButtonPaddingX
-(NSInteger)menuButtonPadding
{
    NSInteger mustWidth = kXHMenuButtonStarX *2;
    for (int i =0;  i < self.menus.count; i++) {
        XHMenu *menu = self.menus[i];
        CGSize buttonSize = [menu.title sizeWithFont:menu.titleFont constrainedToSize:CGSizeMake(MAXFLOAT, CGRectGetHeight(self.bounds) - 10) lineBreakMode:NSLineBreakByCharWrapping];
        mustWidth += buttonSize.width;
    }
    if (self.frame.size.width > (mustWidth + (self.menus.count -1) * kXHMenuButtonPaddingX)) {
        return (self.frame.size.width - mustWidth) /self.menus.count;
        //return (self.frame.size.width - mustWidth) /(self.menus.count -1); //| xx   xxx   xxx |
    }
    else return kXHMenuButtonPaddingX;
}
