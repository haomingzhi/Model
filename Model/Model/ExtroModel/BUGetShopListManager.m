...
@implementation BUGetShopListManager
...
{
     return   [self getList:key typeId:typeId orderBy:orderBy longitude:longitude  latitude:latitude cityId:cityId observer:observer  callback:callback extraInfo:nil ];
}
...
{
     return [self getData:@{@"key":key?:@"",@"typeId":typeId?:@"",@"orderBy":orderBy?:@"",@"longitude":longitude?:@"",@"latitude":latitude?:@"",@"cityId":cityId?:@""} observer:observer callback:callback extraInfo:extraInfo];
}
...
-(BOOL)getDataList:(BUPageInfo *)pageinfo  observer:(id) observer callback:(SEL) callback  extraInfo:(NSDictionary*)extraInfo
{
     ...
     NSInvocation *input = BSGetInvocationFromSel(self, @selector(getDataListInput:typeId:orderBy:longitude:latitude:cityId:pageIndex:pageSize:));
     ...
     [input setArgument:&pageIndex atIndex:8];
     [input setArgument:&pageSize atIndex:9];
     [input retainArguments];
     return [self sendRequest:[NSString stringWithFormat:@"%@%@%@",BU_BUSINESS_DOMAIN_SHOP,BU_BUSINESS_GetShopList,@""]
                         head:nil
                       method:@"POST"
                        async:YES
...
-(NSData *)getDataListInput:(NSString*)key typeId:(NSString *)typeId orderBy:(NSString *)orderBy longitude:(NSString *)longitude  latitude:(NSString *)latitude cityId:(NSString *)cityId pageIndex:(NSString *)pageIndex pageSize:(NSString *)pageSize
{
     NSString *request = [NSString stringWithFormat:@"pageIndex=%@&pageSize=%@&key=%@&typeId=%@&orderBy=%@&longitude=%@&latitude=%@&cityId=%@&%@",pageIndex,pageSize,key,typeId,orderBy,longitude,latitude,cityId,[self getBaseUrlParem]];
...
-(NSError *)getDataListOutput:(NSData *)recvData ok:(BOOL)ok input:(NSInvocation *)input
{
     ...
     BUGetShopListManager *msgManager = [[BUGetShopListManager alloc] init];
