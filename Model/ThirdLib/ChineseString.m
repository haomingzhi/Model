...
+(NSMutableArray*)ReturnSortChineseArrar:(NSArray*)stringArr
{
    ...
    {
        BUBespokeCity *area=stringArr[i];
        ChineseString *chineseStr = [[ChineseString alloc]init];
        chineseStr.string = area.name;
        //        area.currentCity.chineseString.string = area.currentCity.cityName;
        if(chineseStr.string == nil){
            chineseStr.string = @"";
        }
        //去除两端空格和回车
       chineseStr.string  = [chineseStr.string stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
...
//        area.currentCity.chineseString.string = [ChineseString RemoveSpecialCharacter:area.currentCity.chineseString.string];
        ...
        NSString *initialStr = [chineseStr.string length]?[chineseStr.string substringToIndex:1]:@"";
        if ([predicate evaluateWithObject:initialStr])
        {
            NSLog(@"chineseString.string==%@",chineseStr.string);
            //首字母大写
            chineseStr.pinYin = [chineseStr.string capitalizedString] ;
        }else{
            if(![chineseStr.string isEqualToString:@""]){
                NSString *pinYinResult = [NSString string];
                for(int j=0;j<chineseStr.string.length;j++){
                    if ([chineseStr.string isEqualToString:@"长沙市"]) {
                        NSString *singlePinyinLetter =@"C";
                        pinYinResult = [pinYinResult stringByAppendingString:singlePinyinLetter];
                    }
                    else if ([chineseStr.string isEqualToString:@"厦门市"]){
                        NSString *singlePinyinLetter =@"X";
                        pinYinResult = [pinYinResult stringByAppendingString:singlePinyinLetter];
                    }
                    else{
                        NSString *singlePinyinLetter = [[NSString stringWithFormat:@"%c",
                                                         
                                                         pinyinFirstLetter([chineseStr.string characterAtIndex:j])]uppercaseString];
        ...
        [chineseStringsArray addObject:chineseStr];
    }
    //按照拼音首字母对这些Strings进行排序
    [chineseStringsArray sortUsingDescriptors:sortDescriptors];
    NSArray *jarr = [chineseStringsArray sortedArrayUsingComparator:^NSComparisonResult(id  _Nonnull obj1, id  _Nonnull obj2) {
        BUCity *area =[BUCity new];
        area=obj1;
        BUCity *area1 =[BUCity new];
        area1=obj2;
