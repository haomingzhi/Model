...
- (void)dealloc {
     ...
     [_photo.toView removeObserver:self forKeyPath:@"Image"];
}
...
- (void)setPhoto:(ZLPhotoPickerBrowserPhoto *)photo{
     ...
     if (photo.photoURL.absoluteString.length) {
          ...
          [photo.toView addObserver:self forKeyPath:@"Image" options:0 context:nil];
          self.isLoadingDone = YES;
          [self displayImage];
     }
}
...
-(void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context
{
     if ([keyPath isEqualToString:@"Image"]) {
          _photo.aspectRatioImage = [(UIImageView *)object image];
          _photo.photoImage = _photo.toView.image;
          _photo.thumbImage = _photo.toView.image;
          _photoImageView.image = _photo.toView.image;
          [self displayImage];
     }
}
