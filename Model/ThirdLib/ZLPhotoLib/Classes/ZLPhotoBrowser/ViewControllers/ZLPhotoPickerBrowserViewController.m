...
- (void)showToView{
    ...
    if (self.status == UIViewAnimationAnimationStatusFade){
        ...
        if (thumbImage == nil) {
            ...
            if ([thumbImage isKindOfClass:[BUImageRes class] ]) {
                [(BUImageRes *)thumbImage setIsValid:YES];
                [(BUImageRes *)thumbImage  displayRemoteImage:imageView imageName:@"" thumb:YES];
            }
            else
            {
            imageView.image = thumbImage;
    ...
    self.disMissBlock = ^(NSInteger page){
        ...
        [UIView animateWithDuration:0.35 animations:^{
            if (weakSelf.status == UIViewAnimationAnimationStatusFade){
                ...
                mainView.backgroundColor = [UIColor colorWithRed:0.0 green:0.0 blue:0.0 alpha:0.0];
                mainBgView.backgroundColor = [UIColor colorWithRed:0.0 green:0.0 blue:0.0 alpha:0.0];
               CGRect ct = originalFrame;
                ct.origin.y += 64;//修正高版本y上偏移
                imageView.frame = ct;
            }
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(cacheImgCallBack:) name:@"ImgCallBack" object:nil];
...
-(void)cacheImgCallBack:(NSNotification *)noti
{
    UIImage *img = noti.userInfo[@"img"];
    UIImageView *imgV = noti.userInfo[@"imgV"];
    NSInteger row = [noti.userInfo[@"row"] integerValue];
    if (row >= self.photos.count) {
        return;
    }
   ZLPhotoPickerBrowserPhoto *photo = self.photos[row];
    if (![imgV isEqual:photo.toView]) {
        return;
    }
