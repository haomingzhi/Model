...
{
     ZLPhotoPickerAssetsViewController *assetsVc;
     //     ZLPhotoPickerGroup *gp ;
}
...
- (void) setupButtons{
     ...
     [btn setImage:[UIImage imageContentWithFileName:@"nav_back"] forState:UIControlStateNormal];
     [btn addTarget:self action:@selector(back) forControlEvents:UIControlEventTouchUpInside];
     UIBarButtonItem *barItem = [[UIBarButtonItem alloc] initWithCustomView:btn];//[[UIBarButtonItem alloc] initWithTitle:@"" style:UIBarButtonItemStyleDone target:self action:@selector(back)];
...
- (void) jump2StatusVc{
     ...
     assetsVc = [[ZLPhotoPickerAssetsViewController alloc] init];
     assetsVc.selectPickerAssets = self.selectAsstes;
     assetsVc.assetsGroup = gp;
     assetsVc.topShowPhotoPicker = self.topShowPhotoPicker;
     assetsVc.isCanAutoChangeCheck = self.isCanAutoChangeCheck;
     assetsVc.isHeadPortraitScreenshot = self.isHeadPortraitScreenshot;
...
-(void)setIsShowTakePhoto:(BOOL)isShowTakePhoto
{
     ...
     assetsVc.isShowTakePhoto = isShowTakePhoto;
}
-(void)reloadPhotos
{
     //   assetsVc.assetsGroup = gp;
