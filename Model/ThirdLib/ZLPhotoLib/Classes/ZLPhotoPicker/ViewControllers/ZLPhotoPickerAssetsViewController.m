...
- (UIButton *)doneBtn{
     if (!_doneBtn) {
          ...
          [rightBtn setTitleColor:kUIColorFromRGB(color_1) forState:UIControlStateNormal];
...
- (void)setSelectPickerAssets:(NSArray *)selectPickerAssets{
     ...
     self.doneBtn.userInteractionEnabled = (count > 0);
}
...
-(void)setIsShowTakePhoto:(BOOL)isShowTakePhoto
{
     ...
     self.collectionView.isShowTakePhoto = isShowTakePhoto;
...
-(void)setIsHeadPortraitScreenshot:(BOOL)isHeadPortraitScreenshot
{
     ...
     self.collectionView.isHeadPortraitScreenshot = _isHeadPortraitScreenshot;
}

-(void)setIsCanAutoChangeCheck:(BOOL)isCanAutoChangeCheck
{
     _isCanAutoChangeCheck = isCanAutoChangeCheck;
     self.collectionView.isCanAutoChangeCheck = _isCanAutoChangeCheck;
}
...
- (ZLPhotoPickerCollectionView *)collectionView{
     if (!_collectionView) {
          ...
          collectionView.delegateRootVc =_delegate;
          // 时间置顶
          ...
          collectionView.contentInset = UIEdgeInsetsMake(5, 0,TOOLBAR_HEIGHT* 2, 0);
...
-(void)setGroupVc:(ZLPhotoPickerGroupViewController *)groupVc
{
     _groupVc = groupVc;
     if ([self.groupVc.delegate respondsToSelector:@selector(pressTitleButton:)]) {
          UIButton *btn = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 100, 20)];
          [btn setTitle:@"相机胶卷" forState:UIControlStateNormal];
          UIImageView *img = [[UIImageView alloc] initWithFrame:CGRectMake(88, 7, 10, 6)];
          img.image = [UIImage imageNamed:@"downChoice@2x---Assistor"];
          [btn addSubview:img];
          //         [btn setTitleColor:kUIColorFromRGB(color_0x212121) forState:UIControlStateNormal];
          [btn addTarget:self action:@selector(pressTitleButton:) forControlEvents:UIControlEventTouchUpInside];
     ...
     else
     {
          ...
          //        self.title = @"图片";//assetsGroup.groupName;
     }
}
...
-(void)pressTitleButton:(UIButton *)btn
{
     ...
     [self.groupVc.delegate performSelector:@selector(pressTitleButton:) withObject:btn];
}
...
- (void) setupButtons{
     ...
     [btn addTarget:self action:@selector(backbb) forControlEvents:UIControlEventTouchUpInside];
...
-(void)backbb
{
     [self dismissViewControllerAnimated:YES completion:nil];//退出相册还需修改
     [[NSNotificationCenter defaultCenter] postNotificationName:@"cancelTakePhoto" object:nil];
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info{
     ...
     if ([UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera]) {
          ...
          ALAssetsLibraryWriteImageCompletionBlock completeBlock = ^(NSURL *assetURL, NSError *error){
               if (!error) {
                    //                ZLPhotoAssets *zl = [[ZLPhotoAssets alloc] init];
                    //                ALAsset *al = [[ALAsset alloc] init];

                    //              NSString *  imageURL = [NSString stringWithFormat:@"%@",assetURL];
                    [self.selectAssets addObject:assetURL];
                    self.collectionView.selectAssets = self.selectAssets;
...
- (void) setupToorBar{
     ...
     toorBar.items = @[leftItem,fiexItem];
     toorBar.hidden = YES;
}
...
{
     [self setupAssets];
     [self setTopShowPhotoPicker:YES];
     //                if ([[self.selectAssets lastObject] isKindOfClass:[NSURL class]]) {
     //                    return ;
     //                }
     [_collectionView.selectsIndexPath removeAllObjects];
     self.selectAssets = _collectionView.selectAssets;
     NSInteger count = self.selectAssets.count;
     self.makeView.hidden = !count;
