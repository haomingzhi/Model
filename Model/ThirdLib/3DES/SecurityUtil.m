...
+(NSString*)decryptAESNString:(NSString *)str app_key:(NSString *)key
{
...
//    NSString *decryptStr = [[NSString alloc] initWithData:decryStr encoding:NSUTF8StringEncoding];
    ...
    NSData *decryData = [EncryptData1 AES128DecryptWithKey:key];
    //将解了密码的nsdata转化为nsstring
...
+(NSData*)decryptAESWithData:(NSData*)data  app_key:(NSString*)key
{
    //使用密码对data进行解密
    NSData *decryData = [data AES128DecryptWithKey:key];
    return decryData;
