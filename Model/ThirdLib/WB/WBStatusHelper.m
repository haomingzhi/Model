...
+ (NSMutableDictionary *)_emoticonDicFromPath:(NSString *)path {
    ...
    if (!group) {
        NSString *plistPath = [path stringByAppendingPathComponent:@"einfo.plist"];
...
+ (NSArray *)emoticonGroups {
    ...
    dispatch_once(&onceToken, ^{
        ...
        NSDictionary * dic =packages[0];
        NSArray *QQ =@[dic];
        groups = (NSMutableArray *)[NSArray modelArrayWithClass:[WBEmoticonGroup class] json:QQ];
        ...
        for (int i = 0, max = (int)groups.count; i < max; i++) {
            ...
            NSString *infoPlistPath = [path stringByAppendingPathComponent:@"einfo.plist"];
